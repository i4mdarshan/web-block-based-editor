...
const SidebarLayoutHeader = React.forwardRef<
  HTMLDivElement,
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
      'flex h-[var(--header-height)] flex-none items-center gap-1 bg-background p-1 md:px-4',
      className
    )}
    {...props}
  />
))
SidebarLayoutHeader.displayName = 'SidebarLayoutHeader'

interface SidebarLayoutBodyProps extends React.HTMLAttributes<HTMLDivElement> {
  fixedHeight?: boolean
}

const SidebarLayoutBody = React.forwardRef<HTMLDivElement, SidebarLayoutBodyProps>(
...
SidebarLayoutBody.displayName = 'SidebarLayoutBody'

export { SidebarLayout, SidebarLayoutHeader, SidebarLayoutBody }